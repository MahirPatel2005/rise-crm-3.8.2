<?php
echo form_open(get_uri("paystack_payment_method/initiate_payment_process"), array("id" => "paystack-checkout-form", "class" => "float-start", "role" => "form"));
?>
<input type="hidden" name="invoice_id" value="<?php echo $invoice_id; ?>" />
<input type="hidden" name="payment_amount" value="<?php echo to_decimal_format($balance_due); ?>"  id="paystack-payment-amount-field" />
...
<input type="hidden" name="description" value="<?php echo app_lang("pay_invoice"); ?>: (<?php echo to_currency($balance_due, $currency . " "); ?>)" id="paystack_description" />

<button type="button" id="paystack-payment-button" class="btn btn-primary mr15 spinning-btn"><?php echo get_array_value($payment_method, "pay_button_text"); ?></button>
<?php echo form_close(); ?>
...
<?php } ?>
        ...
        $("#payment-amount").on("change", function () {
            //changed the amount. update the description on paystack payment form
            var value = $(this).val();
            $("#paystack_description").val(payInvoiceText + " (" + toCurrency(unformatCurrency(value), currency) + ")");

            //change paystack payment amount field value as inputed/ don't use unformatCurrency we'll do it in controller
            $("#paystack-payment-amount-field").val(value);

            //check minimum payment amount and show/hide payment button
            if (value < minimumPaymentAmount) {
                $("#paystack-payment-button").hide();
            } else {
                $("#paystack-payment-button").show();
            }

        });

        $("#paystack-checkout-form").appForm({
            isModal: false,
            showLoader: false,
