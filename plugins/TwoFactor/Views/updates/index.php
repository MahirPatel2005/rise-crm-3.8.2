<div class="modal-body clearfix">
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div id="twofactor-app-update-container" class="card-body font-14">
                <p>
                ...
                <?php if (count($installable_updates) || count($downloadable_updates)) { ?>
                    ...
                    <script type='text/javascript'>
                        ...
                        $(document).ready(function () {
                            appAlert.warning("Please backup all files and database before start the installation.", {container: "#twofactor-app-update-container", animate: false});
                ...
                <div class="b-t pt15 mt10">
                    <?php echo anchor("twofactor_updates/systeminfo", "Php Info", array("class" => "btn btn-warning", "target" => "_blank")); ?>
                </div>
    ...
    $(document).ready(function () {
        var startDownload = function () {
            ...
            if ($link.length) {
                ...
                $.ajax({
                    url: "<?php echo_uri("twofactor_updates/download_updates/"); ?>" + "/" + version + "/" + salt,
                    dataType: "json",
        ...
        $('body').on('click', '.do-update', function () {
            ...
            $("#twofactor-app-update-container").html("<h3><span class='download-loader-lg spinning-btn spinning'></span> Installing version - " + version + ". Please wait... </h3>");
            $.ajax({
                url: "<?php echo_uri("twofactor_updates/do_update/"); ?>" + "/" + version,
                dataType: "json",
                success: function (response) {
                    $("#twofactor-app-update-container").html("");
                    if (response.success) {
                        appAlert.success(response.message, {container: "#twofactor-app-update-container", animate: false});
                    } else {
                        appAlert.error(response.message, {container: "#twofactor-app-update-container", animate: false});
                    }
                    setTimeout(function () {
                        location.reload();
                    }, 2000);
...
<?php if (isset($error)) { ?>
            appAlert.error("<?php echo $error; ?>", {container: "#twofactor-app-update-container", animate: false});
<?php } ?>
