...
class Skype_Meetings extends Security_Controller {

    protected $Skype_meetings_model;

    function __construct() {
        parent::__construct();
        if ($this->login_user->user_type === "client" && !get_skype_integration_setting("client_can_access_meetings")) {
            app_redirect("forbidden");
        }

        $this->Skype_meetings_model = new \Skype_Integration\Models\Skype_meetings_model();
    }

    function index() {
        return $this->template->rander('Skype_Integration\Views\skype_meetings\index');
    }

    private function can_manage_meetings() {
        if (!can_manage_skype_integration()) {
            app_redirect("forbidden");
        }
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Skype_meetings_model->get_one($id);
        ...
        return $this->template->view('Skype_Integration\Views\skype_meetings\modal_form', $view_data);
    }

    private function get_client_contacts_dropdown() {
        $contacts_dropdown = array();

        $contacts = $this->Skype_meetings_model->get_client_contacts_list()->getResult();
    ...
    function list_data() {
        ...
        $list_data = $this->Skype_meetings_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_row($data) {
        ...
        $status = "<span class='badge bg-primary'>" . app_lang("skype_integration_upcoming") . "</span>";
        ...
        if ($data->start_time > subtract_period_from_date($current_utc_time, 1, "hours", "Y-m-d H:i:s") && $data->start_time < $current_utc_time) { //recent: less than 1 hour past
            $status = "<span class='badge bg-secondary'>" . app_lang("skype_integration_recent") . "</span>";
        } else if ($data->start_time < $current_utc_time) { //past: more than 1 hour past
            $status = "<span class='badge bg-success'>" . app_lang("skype_integration_past") . "</span>";
        }
        ...
        $row_data = array(
            ...
            anchor($data->join_url, app_lang("skype_integration_join_meeting") . "<i data-feather='external-link' class='icon-16 ml10'></i>", array("target" => "_blank")),
            $status,
            modal_anchor(get_uri("skype_meetings/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('skype_integration_edit_meeting'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('skype_integration_delete_meeting'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("skype_meetings/delete"), "data-action" => "delete-confirmation"))
        );
    ...
    function save() {
        ...
        $meeting_data = $Skype_Integration->save_event($data, $id);
        if (!$meeting_data) {
            show_404();
        }
        ...
        $save_id = $this->Skype_meetings_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        $meeting_info = $this->Skype_meetings_model->get_one($id);

        if ($this->Skype_meetings_model->delete($id)) {
            //if there has event associated with this on api calendar, delete that too
            if (get_skype_integration_setting("integrate_skype") && get_skype_integration_setting('skype_authorized') && $meeting_info->event_id) {
                $Skype_Integration = new Skype_Integration();
                $Skype_Integration->delete($meeting_info->event_id);
            }
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Skype_meetings_model->get_details($options)->getRow();
