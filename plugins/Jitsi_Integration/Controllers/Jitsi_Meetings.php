...
class Jitsi_Meetings extends Security_Controller {

    protected $Jitsi_meetings_model;

    function __construct() {
        parent::__construct();
        if ($this->login_user->user_type === "client" && !get_jitsi_integration_setting("client_can_access_meetings")) {
            app_redirect("forbidden");
        }

        $this->Jitsi_meetings_model = new \Jitsi_Integration\Models\Jitsi_meetings_model();
    }

    function index() {
        return $this->template->rander('Jitsi_Integration\Views\jitsi_meetings\index');
    }

    private function can_manage_meetings() {
        if (!can_manage_jitsi_integration()) {
            app_redirect("forbidden");
        }
    }
    ...
    function modal_form() {
        ...
        $model_info = $this->Jitsi_meetings_model->get_one($id);
        ...
        return $this->template->view('Jitsi_Integration\Views\jitsi_meetings\modal_form', $view_data);
    }

    private function get_client_contacts_dropdown() {
        $contacts_dropdown = array();

        $contacts = $this->Jitsi_meetings_model->get_client_contacts_list()->getResult();
    ...
    function list_data() {
        ...
        $list_data = $this->Jitsi_meetings_model->get_details($options)->getResult();
        $result = array();
    ...
    private function _make_row($data) {
        ...
        $status = "<span class='badge bg-primary'>" . app_lang("jitsi_integration_upcoming") . "</span>";
        ...
        if ($data->start_time > subtract_period_from_date($current_utc_time, 1, "hours", "Y-m-d H:i:s") && $data->start_time < $current_utc_time) { //recent: less than 1 hour past
            $status = "<span class='badge bg-secondary'>" . app_lang("jitsi_integration_recent") . "</span>";
        } else if ($data->start_time < $current_utc_time) { //past: more than 1 hour past
            $status = "<span class='badge bg-success'>" . app_lang("jitsi_integration_past") . "</span>";
        }
        ...
        $row_data = array(
            ...
            anchor("https://meet.jit.si/" . strtolower(str_replace(" ", "-", trim($data->title))) . "-" . $data->join_key, app_lang("jitsi_integration_join_meeting") . "<i data-feather='external-link' class='icon-16 ml10'></i>", array("target" => "_blank")),
            $status,
            modal_anchor(get_uri("jitsi_meetings/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('jitsi_integration_edit_meeting'), "data-post-id" => $data->id))
            . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('jitsi_integration_delete_meeting'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("jitsi_meetings/delete"), "data-action" => "delete-confirmation"))
        );
    ...
    function save() {
        ...
        //save user_id and join key only on insert and it will not be editable
        if (!$id) {
            $data["created_by"] = $this->login_user->id;
            $data["join_key"] = strtolower(make_random_string(20));
        }

        $save_id = $this->Jitsi_meetings_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->Jitsi_meetings_model->delete($id)) {
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Jitsi_meetings_model->get_details($options)->getRow();
