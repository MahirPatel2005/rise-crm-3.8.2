...
namespace Jitsi_Integration\Models;

use App\Models\Crud_model;

class Jitsi_meetings_model extends Crud_model {

    protected $table = null;

    function __construct() {
        $this->table = 'jitsi_meetings';
        parent::__construct($this->table);
    }

    function get_details($options = array()) {
        $jitsi_meetings_table = $this->db->prefixTable('jitsi_meetings');
        $users_table = $this->db->prefixTable('users');

        $where = "";
        $id = get_array_value($options, "id");
        if ($id) {
            $where .= " AND $jitsi_meetings_table.id=$id";
        }

        $current_utc_time = get_current_utc_time();
        $upcoming_only = get_array_value($options, "upcoming_only");
        if ($upcoming_only) {
            $where .= " AND $jitsi_meetings_table.start_time>'$current_utc_time'";
        }
        ...
        if ($statuses) {
            ...
            foreach ($statuses_array as $status) {
                ...
                if ($status === "upcoming") {
                    $statuses_where .= " $jitsi_meetings_table.start_time>'$current_utc_time'";
                } else if ($status === "recent") {
                    $statuses_where .= " ($jitsi_meetings_table.start_time>'$subtract_one_hour' AND $jitsi_meetings_table.start_time<'$current_utc_time') ";
                } else if ($status === "past") {
                    $statuses_where .= " $jitsi_meetings_table.start_time<'$subtract_one_hour' ";
                }
            }
        ...
        if (!$is_admin && $user_id) {
            ...
            if ($team_ids) {
                ...
                foreach ($teams_array as $team_id) {
                    $team_search_sql .= " OR (FIND_IN_SET('team:$team_id', $jitsi_meetings_table.share_with_team_members)) ";
                }
            }
            ...
            if ($is_client) {
                ...
                $where .= " AND (FIND_IN_SET('all', $jitsi_meetings_table.share_with_client_contacts) OR FIND_IN_SET('contact:$user_id', $jitsi_meetings_table.share_with_client_contacts))";
            } else {
                //searh for user and teams
                $where .= " AND ($jitsi_meetings_table.created_by=$user_id 
                OR $jitsi_meetings_table.share_with_team_members='all'
                    OR (FIND_IN_SET('member:$user_id', $jitsi_meetings_table.share_with_team_members))
                        $team_search_sql
                        )";
            }
        ...
        $sql = "SELECT $jitsi_meetings_table.*, 
        CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_name, $users_table.image AS created_by_avatar
        FROM $jitsi_meetings_table
        LEFT JOIN $users_table ON $users_table.id = $jitsi_meetings_table.created_by
        WHERE $jitsi_meetings_table.deleted=0 $where";
